...
- PostgreSQL：五份 migration 均通过 PostgreSQL AST 解析；完整执行回放仍需 Docker/隔离数据库。
- Android：JVM tests、debug APK、androidTest 编译通过；应用 ID 仪器测试已在 API 35 设备通过。
...
4. 管理台写操作目前保持禁用，直到完成线上旧数据映射；读路径已接真实 Supabase，未配置时才显示演示数据。
5. Android 社区与市场已接 canonical REST/RPC 和加载、空、错误状态；消息、订单以及迁移后的真实账号联调仍待隔离环境验证。
