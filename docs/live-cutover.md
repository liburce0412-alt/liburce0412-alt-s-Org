...
2026-08-22 已通过官方 Supabase MCP 将 11 份版本化 migration 应用到生产项目。用户明确不保留旧业务数据，因此空的旧业务表和 `legacy` schema 已清理；现有 2 个 Auth 身份仍保留并补建 profile，避免误删登录账号。旧头像文件已删除，Storage 只剩空目录占位元数据。
...
4. 2 个 Auth 用户均已补建 profile；4 条成就定义种子已写入，其他业务表为空。
...
3. 在空白 Supabase 容器完整回放 11 份 migration，并补充并发 RPC 集成测试。
